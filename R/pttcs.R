# ==============================================================================
# --- Inference for Progressive Type-II Censoring Schemes ---
# ==============================================================================
#
#' @title Inference for Progressive Type-II Censoring Schemes
#'
#' @description `pttcs` computes the Maximum Likelihood Estimates (MLE), variances,
#' standard errors, and asymptotic 95\% confidence intervals for the parameters
#' of any user-defined lifetime distribution under a Progressive Type-II
#' Censoring scheme.
#'
#' @param x_obs A numeric vector containing the completely observed failure times (ordered observations).
#' @param R A non-negative integer vector specifying the progressive censoring scheme (number of units removed at each failure time). Must have the same length as `x_obs`.
#' @param par_start A numeric vector of initial parameter values for the optimization process.
#' @param pdf_f A user-defined function calculating the Probability Density Function (PDF) of the distribution. Must accept arguments `(x, par)`.
#' @param cdf_f A user-defined function calculating the Cumulative Distribution Function (CDF) of the distribution. Must accept arguments `(x, par)`.
#' @param opt_method A character string specifying the optimization method. Options include `"auto"` (runs all available algorithms), `"BFGS"`, `"GA"`, `"DE"`, `"PSO"`, etc. Default is `"auto"`.
#' @param maxiter An integer indicating the maximum number of iterations or generations for the meta-heuristic optimizers. Default is `200`.
#'
#' @details The function transforms parameters into the logarithmic scale during optimization to ensure positivity constraints. Standard errors are computed using the inverse of the Hessian matrix, with a fallback to the Moore-Penrose pseudoinverse if inversion fails.
#'
...
#'
#' # Run the Progressive Type-II Censoring Inference
#' result <- pttcs(x_obs      = obs_data,
#'                 R          = user_scheme,
...
#' @export
#'
pttcs <- function(x_obs, R, par_start, pdf_f, cdf_f, opt_method = "auto", maxiter = 200) {
